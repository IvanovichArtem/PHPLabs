...
try {
    // Получаем список таблиц кроме таблицы user
    $tables_stmt = $pdo->query("SELECT table_name FROM information_schema.tables WHERE table_schema = 'bookstore' AND table_name != 'user'");
                    ...
                    <?php if (in_array($column, $foreign_columns)): ?>
                        <select name="<?= $column ?>" class="form-select mb-3" required>
                            <?php foreach ($foreign_data[$column] as $option): ?>
                                <option
                                    value="<?= $option[$foreign_keys[array_search($column, $foreign_columns)]['REFERENCED_COLUMN_NAME']] ?>">
                                    <?= $option['name'] ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
